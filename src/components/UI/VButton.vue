...
const props = defineProps(['accent', 'type', 'ariaLabel', 'center']);
</script>

<template>
	<button class="button" :class="{ accent: props.accent, center: props.center }" :type="props.type" :aria-label="props.ariaLabel"
		@click="emits('click')">
		<slot name="icon"></slot>
		<slot name="text"></slot>
	</button>
